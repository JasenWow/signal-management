...
CREATE TABLE IF NOT EXISTS messages (
  ...
  frame_size INTEGER NOT NULL, byte_order TEXT NOT NULL DEFAULT 'big',
...
CREATE TABLE IF NOT EXISTS signals (
  ...
  start_bit INTEGER NOT NULL, bit_length INTEGER NOT NULL,
  byte_order TEXT NOT NULL DEFAULT 'big', factor REAL NOT NULL DEFAULT 1.0,
  offset REAL NOT NULL DEFAULT 0.0, unit TEXT NOT NULL DEFAULT '',
  minimum REAL, maximum REAL,
  value_table_id TEXT REFERENCES value_tables(id) ON DELETE SET NULL,
  color TEXT NOT NULL DEFAULT '#10B981', sort_order INTEGER NOT NULL DEFAULT 0,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS value_table_entries (
  id TEXT PRIMARY KEY, value_table_id TEXT NOT NULL REFERENCES value_tables(id) ON DELETE CASCADE,
  raw_value INTEGER NOT NULL, display_value TEXT NOT NULL,
  description TEXT NOT NULL DEFAULT '', sort_order INTEGER NOT NULL DEFAULT 0
);
CREATE TABLE IF NOT EXISTS versions (
  id TEXT PRIMARY KEY, message_id TEXT REFERENCES messages(id) ON DELETE SET NULL,
  parent_id TEXT REFERENCES versions(id), message TEXT NOT NULL DEFAULT '',
  snapshot TEXT NOT NULL, diff TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
  ...
  for (const migration of MIGRATIONS) {
    ...
    if (!row) {
      if (raw && typeof raw.exec === 'function') {
        ...
        db.run(sql.raw(migration.up))
      }
      db.run(sql`INSERT INTO _migrations (name) VALUES (${migration.name})`)
    }
  }
}
